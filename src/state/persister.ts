...
/**
 * Wipe the persisted cache without holding a long-lived handle. Used
 * by the TUI's "clear all data" action (`clearAll` in `state/hooks.ts`,
 * which `yield*`s this directly rather than reaching for its own
 * `Effect.runSync`); opens a short-lived connection, truncates,
 * closes. Handles the "db never created" case via `existsSync`.
 */
export function clearPersistedCache(dbPath: string): Effect.Effect<void> {
...
// Purely synchronous builder — no I/O worth suspending in an Effect, so
// this stays a plain function rather than an `Effect.sync` wrapped in
// its own `Effect.runSync`.
