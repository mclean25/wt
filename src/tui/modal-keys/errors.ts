...
/**
 * Keys for the auto-popping error overlay. j/k scrolling rides the
 * focused `<scrollbox>` inside the panel (same as help/perf), so this
 * only owns the explicit affordances. No trigger re-press leg — the
 * overlay has no opening key (it pops on capture), and the modal UX
 * rules say not to invent one.
 *
