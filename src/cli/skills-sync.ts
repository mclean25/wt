 ...
 */
export const runSkillsSync = Effect.fn("runSkillsSync")(function* (mode: SyncMode) {
  ...
  yield* withAsyncFileLock(
    ...
    Effect.gen(function* () {
      ...
      for (const r of toApply) {
        const result = yield* Effect.result(applyReport(r));
        if (result._tag === "Success") {
          applied.push(r);
          console.log(`${green("✓")} ${bold(r.unit.name)} ${dim("→")} ${targetLabel(r)}`);
        } else {
          failures++;
          console.error(red(`✗ ${r.unit.name}: ${result.failure.message}`));
          log.error(result.failure, { unit: r.unit.name });
        }
      }
      ...
      if (roots.length > 0) {
        ...
        // Never fails — a failed root is reported per-root in the results.
        const results = yield* regenRulesync(roots);
