...
import { confirm as askYesNo, ask as askText, isInteractive } from "./prompt.ts";
...
function targetLabel(r: UnitReport): string {
  return r.target.kind === "rulesync"
    ? `rulesync ${r.target.rulesync.root}`
    : r.path;
}
 ...
 */
export async function runSkillsSync(mode: SyncMode): Promise<number> {
  const targets = detectTargets();
  if (targets.harnesses.length === 0) {
    if (!mode.startup) console.log(dim("no agent harness dirs found (~/.claude, ~/.codex, ~/.config/opencode) — nothing to install into"));
    return 0;
  }
  ...
  if (mode.startup) {
    ...
    console.log(dim("(answers are remembered; \"no\" never re-asks for the same version. [skills] startup_check = false disables this check)"));
  }
  ...
  for (const [unit, rs] of pending) {
    ...
    if (hasModified) {
      ...
      if (!mode.interactive) {
        if (!mode.force) {
          console.log(yellow(`~ ${unit.name}: local copy differs (not wt-managed) — skipped; use \`wt skills sync ${unit.name} --force\` or run interactively`));
          continue;
        }
        accepted.push(unit);
        continue;
      ...
      if (!(await askYesNo(`${yellow("~")} ${bold(unit.name)}: ${detail}. Overwrite with the wt-managed version?`, false))) {
        tryRemember(() => {
  ...
  if (mode.interactive) {
    const acceptedReports = reports.filter(
      (r) => acceptedSet.has(unitKey(r.unit)) && r.target.kind === "rulesync",
    );
  ...
  for (const unit of accepted) {
    for (const v of unit.vars) {
      ...
      const answer = (await askText(`${cyan("?")} ${v.prompt}: `)).trim();
  ...
  let failures = 0;
  await withAsyncFileLock("__skills_sync__", async () => {
    ...
    for (const r of toApply) {
      try {
        ...
        log.error(err instanceof Error ? err : String(err), { unit: r.unit.name });
      }
    }
    ...
    if (roots.length > 0) {
      ...
      for (const result of await regenRulesync(roots)) {
        if (result.ok) {
 ...
 * and swallowed.
 */
export async function startupSkillsPrompt(): Promise<void> {
