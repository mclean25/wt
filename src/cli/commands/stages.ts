...
const io = operationErrors("wt stages");
...
const removeStage = Effect.fnUntraced(function* (name: string) {
  return yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      return yield* io.promise("wait for sst remove", () => process.exited);
    }),
  );
});

export const run = Effect.fn("wt stages")(function* (argv: string[]) {
  if (hasHelpFlag(argv)) {
    console.log(USAGE);
    return 0;
  }
  ...
  const stages = yield* listSstStages();
  if (!stages) {
  ...
  const { live, orphaned } = yield* categorizeStages(stages, worktreeStages);
