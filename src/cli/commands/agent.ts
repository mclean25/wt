...
async function resolveWorktree(slugOrBranch: string) {
  ...
  const wt =
    all.find((w) => !w.isMain && (w.slug === slug || w.branch === slugOrBranch)) ?? null;
  ...
  return { wt, slugs: new Set(all.map((w) => w.slug)) };
}
...
export async function run(argv: string[]): Promise<number> {
  ...
  const { wt, slugs } = await resolveWorktree(parsed.target);
  if (!wt) {
  ...
  const choice = parsed.harness
    ...
    : await resolveWorktreeHarness(wt.slug, slugs);
  ...
  if (choice.source === "primary-unknown") {
    console.error(
      red(`could not ask tmux which sessions are live — falling back to the ${harness.label} primary`),
    );
  ...
  if (result.delivered === null) {
    ...
    console.log(dim(`delivery confirmed in ${wt.slug}'s ${harness.label} conversation`));
  }
