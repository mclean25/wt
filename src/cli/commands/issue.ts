...
export class IssueCommandError extends Data.TaggedError("IssueCommandError")<{
...
function commandIo<A>(
  operation: string,
  f: () => A,
): Effect.Effect<A, IssueCommandError> {
  return Effect.try({
    try: f,
    catch: (cause) => new IssueCommandError({ operation, cause }),
  });
}
...
export function run(argv: string[]): Effect.Effect<number, IssueCommandError> {
  return Effect.gen(function* () {
    ...
    if (rest[0] === "--id") {
      ...
      yield* commandIo("set tracker issue id", () =>
    ...
    if (rest[0] === "--no-id") {
      ...
      yield* commandIo("clear tracker issue id", () =>
    ...
    if (rest[0] === "--clear-id") {
      yield* commandIo("restore tracker issue id", () =>
    ...
    if (rest[0] === "--clear-gh") {
      yield* commandIo("clear GitHub issue", () =>
    ...
    const slugState = (yield* commandIo("read wt state", readWtState)).slugs[
      wt.slug
    ];
