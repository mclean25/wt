...
import { createWorktree, parseInput, type CreateResult } from "../../core/lifecycle.ts";
...
const io = operationErrors("wt new");

function openBestEffort(path: string): Effect.Effect<void> {
  return openInEditor(path).pipe(
    Effect.catchTag("EditorLaunchError", (error) =>
      Effect.sync(() => console.error(red(causeMessage(error.cause)))),
    ),
  );
}

export const run = Effect.fn("wt new")(function* (argv: string[]) {
  if (hasHelpFlag(argv)) {
    console.log(USAGE);
    return 0;
  }
  ...
  const existing = (yield* listWorktrees()).find((w) => !w.isMain && w.branch === branch);
  if (existing) {
    ...
    if (parsed.gh) {
      yield* io.sync("set GitHub issue", () => setSlugGithubIssue(existing.slug, parsed.gh!));
  ...
  const result: Extract<CreateResult, { ok: true }> | { ok: false; reason: string } = yield* createWorktree(branch, {
    ...
    // letting a plain create failure (bad slug, path exists, ...) propagate
    // as a defect-shaped tagged error.
    Effect.catchTag("LifecycleError", (error) => Effect.succeed({ ok: false as const, reason: error.message })),
  );
  ...
  if (parsed.gh) {
    yield* io.sync("set GitHub issue", () => setSlugGithubIssue(result.slug, parsed.gh!));
  }
