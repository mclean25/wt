...
export class BaseCommandError extends Data.TaggedError("BaseCommandError")<{
...
function commandIo<A>(
  operation: string,
  f: () => A,
): Effect.Effect<A, BaseCommandError> {
  return Effect.try({
    try: f,
    catch: (cause) => new BaseCommandError({ operation, cause }),
  });
}
...
function show(slug: string): Effect.Effect<number, BaseCommandError> {
  return Effect.gen(function* () {
    const entry = (yield* commandIo("read wt state", readWtState)).slugs[slug];
    if (!entry?.baseBranch) {
      console.log(
        dim(
...
function set(
  slug: string,
  ref: string,
): Effect.Effect<number, BaseCommandError> {
  return Effect.gen(function* () {
    const wt = yield* findWorktree(slug);
    if (!wt) {
...
function clear(slug: string): Effect.Effect<number, BaseCommandError> {
  return Effect.gen(function* () {
    const entry = (yield* commandIo("read wt state", readWtState)).slugs[slug];
    if (!entry?.baseBranch) {
    ...
    yield* commandIo("clear fork base", () => setSlugBase(slug, null));
    console.log(
...
export function run(argv: string[]): Effect.Effect<number, BaseCommandError> {
  return Effect.gen(function* () {
    if (hasHelpFlag(argv)) {
      console.log(USAGE);
      return 0;
