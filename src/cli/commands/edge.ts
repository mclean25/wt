...
export class EdgeCommandError extends Data.TaggedError("EdgeCommandError")<{
...
function commandIo<A>(
  operation: string,
  f: () => A,
): Effect.Effect<A, EdgeCommandError> {
  return Effect.try({
    try: f,
    catch: (cause) => new EdgeCommandError({ operation, cause }),
  });
}
...
export function run(argv: string[]): Effect.Effect<number, EdgeCommandError> {
  return Effect.gen(function* () {
    ...
    if (positional.length === 0) {
      const edges = (yield* commandIo("read wt state", readWtState)).edges;
      if (edges.length === 0) {
        if (json) console.log("[]");
        else
          console.log(
    ...
    if (positional[0] === "prune") {
      ...
      const before = (yield* commandIo("read wt state", readWtState)).edges;
      const dead = before.filter((e) => !live.has(e.from) || !live.has(e.to));
      if (dead.length === 0) {
        console.log(dim("No edges with dead endpoints."));
        return 0;
      }
      yield* commandIo("prune merge edges", () => pruneMergeEdges(live));
      for (const e of dead) {
        console.log(
    ...
    if (positional[0] === "rm") {
      ...
      const edges = (yield* commandIo("read wt state", readWtState)).edges;
      const removed = yield* commandIo("read removed worktrees", () =>
      ...
      if (
        yield* commandIo("remove merge edge", () => removeMergeEdge(from, to))
      ) {
    ...
    const by =
      (yield* commandIo("resolve current worktree", () => worktreeAtCwd(wts)))
        ?.slug ?? "fleet";
