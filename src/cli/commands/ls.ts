...
const io = operationErrors("wt ls");
...
export const run = Effect.fn("wt ls")(function* (argv: string[]) {
  if (hasHelpFlag(argv)) {
    console.log(USAGE);
    return 0;
  }
  ...
  const recentRemoved = yield* io.sync("read recently removed worktrees", () =>
    recentlyRemovedWorktrees(new Set(rows.map((w) => w.slug))),
  );

  if (jsonOut) {
    const slugStates = (yield* io.sync("read wt state", readWtState)).slugs;
    const snapshots = yield* collectWorktreeSnapshots(rows);
  ...
  const [prs] = yield* Effect.all(
    [fetchPrs(), fetchOrigin()],
    { concurrency: "unbounded" },
  );
  const statuses = yield* Effect.all(
    rows.map((w) => worktreeStatus(w)),
    { concurrency: 8 },
  );
