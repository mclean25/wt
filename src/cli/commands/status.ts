...
const io = operationErrors("wt status");
...
export const run = Effect.fn("wt status")(function* (argv: string[]) {
  ...
  if (args.kind === "examined") {
    ...
    // event that voids it.
    const sha = yield* revParse("HEAD", target.path);
    if (!sha) {
      console.error(red(`could not resolve HEAD for ${target.slug}`));
      return 2;
    }
    const baseBranch = state.slugs[target.slug]?.baseBranch ?? null;
    const baseSha = yield* baseTipSha(baseBranch);
    const by = agentIdentity();
    const examinedAtMs = yield* Clock.currentTimeMillis;
    yield* io.sync("set examined verdict", () =>
      setSlugExamined(target.slug, {
        sha,
        ...(baseSha ? { baseSha } : {}),
        verdict: args.verdict,
        at: DateTime.formatIso(DateTime.makeUnsafe(examinedAtMs)),
        ...(by ? { by } : {}),
      }),
    );
    console.log(
  ...
  if (args.kind === "clear") {
    yield* io.sync("clear work status", () =>
  ...
  if (args.kind === "amend") {
    ...
    const wrote = yield* io.sync("amend work status", () =>
    ...
    yield* io.sync("write work status log", () =>
  ...
  if (args.kind === "show") {
    const headSha = yield* revParse("HEAD", target.path);
    console.log(
  ...
  const assertedAtMs = yield* Clock.currentTimeMillis;
  const record: WorkStatusRecord = {
    state: args.state,
    at: DateTime.formatIso(DateTime.makeUnsafe(assertedAtMs)),
  };
  ...
  const wrote = yield* io.sync("set work status", () =>
