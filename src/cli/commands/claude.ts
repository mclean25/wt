...
class ClaudeCommandError extends Data.TaggedError("ClaudeCommandError")<{
  readonly operation: "sessions" | "selftest" | "stdin" | "stop";
  readonly cause: unknown;
}> {}

const commandPromise = <A>(
  operation: ClaudeCommandError["operation"],
  run: () => Promise<A>,
): Effect.Effect<A, ClaudeCommandError> =>
  Effect.tryPromise({
    try: run,
    catch: (cause) => new ClaudeCommandError({ operation, cause }),
  });
...
function send(slugOrBranch: string, textArgs: string[]): Effect.Effect<number, ClaudeCommandError> {
  return Effect.gen(function* () {
    ...
    const text = (textArgs.length > 0 ? joined : yield* commandPromise("stdin", () => Bun.stdin.text())).trim();
    if (!text) {
    ...
    Effect.mapError((cause) =>
      cause instanceof ClaudeCommandError ? cause : new ClaudeCommandError({ operation: "sessions", cause }),
    ),
  );
}
...
function stop(slugOrBranch: string): Effect.Effect<number, ClaudeCommandError> {
  return Effect.gen(function* () {
    const slot = SLOT_TARGETS[slugOrBranch] ?? null;
    const wt = slot ? null : yield* findWorktree(slugOrBranch);
    if (!slot && !wt) {
...
export function run(argv: string[]): Effect.Effect<number, ClaudeCommandError> {
  return Effect.gen(function* () {
