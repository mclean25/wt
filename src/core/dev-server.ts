 ...
 * holding nothing.
 */
export async function handleDevGiveUp(slug: string): Promise<void> {
  await captureDevCrashLog(slug);
  const wt = (await listWorktrees()).find((w) => w.slug === slug) ?? null;
  await runDevStopCommand(slug, wt?.path ?? null);
  await run([
