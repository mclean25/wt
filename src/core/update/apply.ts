...
const syncDeps = Effect.fnUntraced(function* (
  fromSha: string,
  toSha: string,
  dependencies: ApplyDependencies,
): Effect.fn.Return<{ ran: boolean; ok: boolean; detail: string }> {
 ...
 */
export const smokeCheckout = Effect.fn("smokeCheckout")(function* (
  dependencies: ApplyDependencies = productionDependencies,
): Effect.fn.Return<{ ok: true } | { ok: false; detail: string }> {
 ...
 */
export const applyWtUpdate = Effect.fn("applyWtUpdate")(function* (
  targetSha: string,
  dependencies: ApplyDependencies = productionDependencies,
): Effect.fn.Return<ApplyResult, never, Scope.Scope> {
  ...
  const deps = before
    ? yield* syncDeps(before, targetSha, dependencies)
  ...
  if (!smoke.ok) {
    ...
    if (before) {
      ...
      yield* Effect.sync(dependencies.clearApplying);
      if (deps.ran) {
        const back = yield* syncDeps(targetSha, before, dependencies);
        if (!back.ok) {
 ...
 */
export const performRollback = Effect.fn("performRollback")(function* (
  targetSha: string,
  now: number,
  dependencies: ApplyDependencies = productionDependencies,
): Effect.fn.Return<
  { ok: true; fromSha: string; depsWarning: string | null } | { ok: false; detail: string },
  never,
  Scope.Scope
> {
  const locked = yield* dependencies.lock;
  ...
  const deps = yield* syncDeps(fromSha, targetSha, dependencies);
