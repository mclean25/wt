...
- **Trigger-key re-press confirms.** Whatever key opens the picker (`l`, `;`, `'`, `!`, `v`, `b`, `u`, `y`, `Shift+F12`) also commits the highlighted row when pressed again (`l l`, `; ;`, `' '`, `! !`, `v v`, `u u`, `y y`) — the `confirm` option.
...
- **`x` kills** where rows represent killable things. Killing something LIVE routes through the shared confirm modal (`killSessionConfirm`), matching the Shift+F10/F11 paths; forgetting a dead ghost stays immediate — there's nothing to lose.
- **Confirm modals cancel on their opening key.** `handleYesNoKey`'s `extraCancelKeys` carries the opener (`d`, `c`, `e`, `E`, `M`, `w`, `x`, `!`) so the muscle-memory toggle works on the destructive path too. Openers that aren't a single bare key (Ctrl+R, Shift+F10/F11, Enter) keep just the universal cancels.
