...
Claude lifecycle and external messaging live in `core/harness/claude/sessions.ts`. A target is the canonical cwd plus its deterministic wt conversation UUID and optional managed name. `ensure` serializes cold starts, reuses the normal detached tmux host, and waits for a live native inbox. Every wt-managed Claude process receives SessionStart/SessionEnd hooks that atomically maintain a private `claude-runtime/registry.json` under the wt cache root; discovery validates socket liveness and also merges Claude's live process registry so crashes, stale files, restarts, and pre-hook sessions heal without trusting either source alone. The registry stores the messaging token with directory mode 0700 and file mode 0600; JSON CLI output never exposes it.

Claude delivery is NDJSON over `CLAUDE_CODE_MESSAGING_SOCKET`. Anthropic's public docs define the socket and lifecycle semantics; the tiny transport adapter uses only the auth and user frame recipe emitted by the Claude Code 2.1.228 implementation, rather than inventing control frames or acknowledgements. It is version-gated to Claude Code 2.1.228 or newer, sends no terminal input, and confirms the prompt in the conversation JSONL before reporting success. Tmux remains only the process and UI host for Claude. `core/harness/session-messaging.ts` is the narrow harness router: Claude uses this native service, while Codex and OpenCode keep their terminal adapters. Claude stop hard-kills the hosted session rather than sending control keys.
