...
| `Ctrl+N` | create on `[remote]`; the worktree appears in its normal section (or Inbox) with a small remote indicator, and F10/F11/F12 route that row's sessions over SSH |
...
| `#` | set the worktree's tracker id — a footer prompt seeded with whatever the row resolves to today; `Enter` saves, an empty line clears back to the slug-parsed id, `Esc` cancels. Validated (`COZ-2185` shape), stored per-slug, and preferred over the slug everywhere: the issue row, `i`/`I`, `{{issue_id}}`, and `requires = ["issue.tracker"]`. This is how a worktree named for the work rather than the ticket gets one |
...
| `! m` | arm/disarm "merge when ready" — a `!` picker row since `M` became the manager palette; fires directly, no confirm. Dispatches on the PR's BASE branch, because two different GitHub features wear that label: a base with a merge queue is enqueued/dequeued (`enqueuePullRequest`), anything else arms classic auto-merge. They are gated differently — classic auto-merge needs the repo's "Allow auto-merge" setting, a queue does not — so guessing wrong fails outright. A queue refuses a PR whose required check has not REPORTED for the head sha, in two wordings that are one situation at two ages: `is expected` (the workflow has not created the check run yet, a ~60s race) and `is in progress` (created, still running). Both clear on their own, so both are retried in the background until the arm takes (`V`-independent; `! m` again cancels a pending arm, and the retry re-sends the same head sha so a push during the wait refuses rather than arming an unseen commit). wt still tries classic auto-merge alongside it, which turns the wait into a done thing on repos that allow auto-merge; every other refusal is reported as-is. Arm-only by construction: on a repo where nothing would block the merge (no protection, no queue), GitHub refuses the classic arm with "clean status" rather than wt ever merging on the spot. **Armed but not queued takes the list's PR slot**, rendering the merge-queue icon without a position in the same colour the details pane's own auto-merge segment uses (`prSlotBadge`). The slot already swaps to icon-plus-position once there IS a queue entry; this is the third case. The details pane keeps the plain PR glyph there, because it has room for the dedicated segment and swapping would print the same icon twice on one line |
