...
- `wt claude send <slug> "<text>"` — ensure and nudge a worktree session (also accepts the `wt`/`main`/`dotfiles`/`manager` repo-level slugs; an archived slug answers with why it is gone). Claude delivery uses the native socket and is confirmed against the target transcript; a non-zero exit means the message is not in that conversation. One exception: a payload that IS a slash command (`/compact …`) goes through the session's pane instead, because the socket carries messages — the receiver frames them as quoted peer text, so a command sent that way arrives as a paragraph about the command and silently does nothing. Give such a payload arguments (`/compact <focus instructions>`, as both palette `m` builtins do); a bare command opens the harness's autocomplete, which swallows the submit. The pane is shared with you, so an unsent draft sitting in that input gets submitted along with the command.
