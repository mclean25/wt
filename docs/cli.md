...
Scope is exactly *do not merge yet*: a mobile release shipping, an upstream branch landing, a hosted change that must be in place **before** this merges. Work that happens **after** the merge — a migration to apply, functions to redeploy — is the note's `OPS:` line and is not a gate; gating on it would make the field mean "read the note", which is what it replaced. The flag is refused on any state but `ready` (still working on it is `working`; blocked on the human to make progress at all is `needs-human`).
